...
import numpy as np
import json
from dubey_regret import DubeyGame, DubeyPlayer, Tensor_Strategy
...
class NNOracle:
    def __init__(self, game: DubeyGame):
        ...
        output_layer = keras.layers.Dense(self.num_outputs, activation='relu')(x)
    ...
    def get_custom_loss(self, debug = False):
        ...
        def custom_loss(y_true, y_pred):
            def per_sample_loss(true, pred):
                ...
                true_mask = np.full((num_players, 4, num_goods), True, dtype=bool)
                ...
                true_bought_quantities = true_trade_amounts[:, 1]
                player_utilities = tf.reduce_sum(preferences * true_bought_quantities, axis=1)
                true_utility = tf.reduce_sum(player_utilities)

                if debug:
                    print("true_bought_quantities: ", true_bought_quantities)
                    print("player_utilities: ", player_utilities)
                    print("true_utility: ", true_utility)

                pred_bought_quantities = pred_trade_amounts[:, 1]
                pred_player_utilities = tf.reduce_sum(preferences * pred_bought_quantities, axis=1)
                pred_utility = tf.reduce_sum(pred_player_utilities)
                
                if debug:
                    print("pred_bought_quantities: ", pred_bought_quantities)
                    print("pred_player_utilities: ", pred_player_utilities)
                    print("pred_utility: ", pred_utility)
                
                return true_utility - pred_utility - 10 * total_oversold_amount
